...
program
	...
	.option('-o, --overwrite', 'Overwrite existing TypeScript files. Default: Enabled');
...
program
	...
	.action(async (inputFile: string, outputPath?: string) => {
		...
		await convertFile(inputFile, outputPath, options.overwrite, options.replace);
	});
...
program
	...
	.action(async (inputDirectory: string, outputDirectory?: string) => {
		const options = program.opts();
		await convertDirectory(inputDirectory, outputDirectory, options.overwrite, options.replace);
	});
 ...
 */
async function saveTypeScriptFile(tsCode: string, outputPath: string, overwrite: boolean, replace: boolean) {
...
async function convertFile(inputFile: string, outputPath?: string, overwrite: boolean = true, replace: boolean = false) {
	try {
 ...
 * @param {boolean} replace - Whether to delete the original JavaScript file after conversion.
 */
async function convertDirectory(inputDirectory: string, outputDirectory?: string, overwrite: boolean = true, replace: boolean = false) {
	try {
