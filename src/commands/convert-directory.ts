...
export const convertDirectory = async (
	...
	try {
		const filteredJsFiles = await Array.fromAsync(
			findFilesRecursivelyStringEndsWith(sourceDirectory, ".js"),
		);
		if (filteredJsFiles.length === 0)
			Logger.error(
				`No JavaScript files found in directory ${sourceDirectory}.`,
			);

		Logger.info(
			`Found ${filteredJsFiles.length} JavaScript files to convert.`,
		);

		for (const javascriptFile of filteredJsFiles) {
			Logger.info(`Converting ${javascriptFile}...`);

			const javascriptFileCode = await readFile(
				appendJSExtension(javascriptFile),
				"utf-8",
			);
